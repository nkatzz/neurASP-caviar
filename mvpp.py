...
class MVPP(object):
        ...
        for ruleIdx,list_of_atoms in enumerate(self.pc):
            for atomIdx, atom in enumerate(list_of_atoms):
                if atom in I:
                    ...
                    # print(atom,self.parameters[ruleIdx][atomIdx])
                # else:
                #     print(atom, "is not in I")
                #     input()
        ...
        if len(models) == 1:
            ...
            for i, cEqualsVi in enumerate(self.pc[ruleIdx]):
                ...
                if cEqualsVi in model:
                    ...
                    # print(cEqualsVi,"is in model")
                    # input()
                else:
                    gradients.append(-1.0)
                    # print(cEqualsVi,"is not in model")
                    # input()
            for i, cEqualsVi in enumerate(self.pc[ruleIdx]):
                gradients[i] = gradients[i]/p
            #     print(gradients[i],cEqualsVi)
            # input()
            ...
            for i, cEqualsVi in enumerate(self.pc[ruleIdx]):
                ...
                for modelIdx, model in enumerate(models):
                    ...
                    if cEqualsVi in model:
                        ...
                        # print(cEqualsVi,"is in the model" ,modelIdx)

                    # if I does not satisfy cEqualsVi
                    else:
                        # print(cEqualsVi,"is not in the model",modelIdx)
                        for atomIdx, atom in enumerate(self.pc[ruleIdx]):
                            if atom in model:
                                # print(atom,"atom is in the model",modelIdx)
    ...
    def gradients_one_obs(self, obs, opt=False):
        ...
        if opt:
            ...
            models = self.find_k_SM_under_obs(obs, k=100)
